...
| |_| || || |_| |  |  _/ ___ \ | < | |
 \___/ \_/ \___/   |_|/_/   \_\_|\_\|_|
    ...
    [bold yellow]Oto Faik Çoklu Tedarikçi Paneli v7.5[/bold yellow]
"""
...
def load_existing_data():
    """Mevcut JSON verisini yükler ve OEM koduna göre indeksler."""
...
def save_data_to_json(db_map):
    """Hafızadaki harita verisini düz liste olarak JSON dosyasına yazar."""
...
def show_status_panel():
    ...
    table.add_row("Çalışma Modu", "[bold cyan]Otomatik Keşif + Mükerrer Engelleme[/bold cyan]")

    console.print(Panel(table, title="[bold blue]📊 Sistem Durum Özeti[/bold blue]", border_style="blue"))

def run_full_scrape():
    console.print("\n[bold green]🚀 ÇOKLU TEDARİKÇİ (AKBAY + OTOERDEM) KAZIMA BAŞLATILIYOR...[/bold green]\n")

    db_map = load_existing_data()
    console.print(f"[bold cyan]ℹ Hafızaya Yüklenen Mevcut Kayıt Sayısı:[/bold cyan] {len(db_map)} Adet\n")

    session = requests.Session()
    new_urls_to_scrape = []

    # 1. AKBAY RENAULT CANLI KATEGORİ KEŞFİ
    console.print("[bold yellow]🌐 1. Akbay Renault Canlı Kategorileri Keşfediliyor...[/bold yellow]")
    akbay_categories = auto_discover_categories(session, AKBAY_DOMAIN)
    console.print(f"  [green]✓[/green] [bold yellow]{len(akbay_categories)}[/bold yellow] adet aktif Akbay Renault kategorisi bulundu.")

    with Progress(console=console) as progress:
        task1 = progress.add_task("[cyan]Akbay Ürün Linkleri Toplanıyor...", total=len(akbay_categories))
        for cat_url in akbay_categories:
            p_urls = collect_product_urls_from_category(session, cat_url, AKBAY_DOMAIN)
            for u in p_urls:
                new_urls_to_scrape.append({"url": u, "site": "akbayrenault"})
            progress.advance(task1)

    # 2. OTO ERDEM CANLI KATEGORİ KEŞFİ
    console.print("\n[bold yellow]🌐 2. Oto Erdem Canlı Kategorileri Keşfediliyor...[/bold yellow]")
    otoerdem_categories = auto_discover_categories(session, OTOERDEM_DOMAIN)
    console.print(f"  [green]✓[/green] [bold yellow]{len(otoerdem_categories)}[/bold yellow] adet aktif Oto Erdem kategorisi bulundu.")

    with Progress(console=console) as progress:
        task2 = progress.add_task("[magenta]Oto Erdem Ürün Linkleri Toplanıyor...", total=len(otoerdem_categories))
        for cat_url in otoerdem_categories:
            p_urls = collect_product_urls_from_category(session, cat_url, OTOERDEM_DOMAIN)
            for u in p_urls:
                new_urls_to_scrape.append({"url": u, "site": "otoerdem"})
            progress.advance(task2)

    console.print(f"\n[bold green]🎯 Toplam Bulunan İşlenecek Bağlantı Sayısı:[/bold green] [bold yellow]{len(new_urls_to_scrape)}[/bold yellow] Adet.")

    if not new_urls_to_scrape:
        console.print("[bold red]HATA:[/bold red] Ürün linki toplanamadı.")
        return

    # 3. KAZIMA VE MÜKERRERLİK KONTROLÜ
    console.print("\n[bold yellow]📦 ADIM 3: Ürün Detayları Çekiliyor ve Mükerrer Kontrolü Yapılıyor...[/bold yellow]")
    ...
    with Progress(
        ...
        task3 = progress.add_task("[green]Veriler İşleniyor...", total=len(new_urls_to_scrape))

        for idx, item in enumerate(new_urls_to_scrape, start=1):
            p_url = item["url"]
            site = item["site"]

            data = scrape_product_details(session, p_url, site)
            if data:
                oem = data["oem_kodu"]
                key = oem if oem != "OEM_BULUNAMADI" else p_url

                if key in db_map:
                    # Var olan kaydın fiyatını ve güncelleme tarihini tazele
                    db_map[key]["fiyat"] = data["fiyat"]
                    db_map[key]["son_guncelleme"] = time.strftime("%Y-%m-%d %H:%M:%S")
                    updated_count += 1
                    console.print(f"  [yellow]↻ Güncellendi (Mevcut OEM):[/yellow] {data['urun_adi'][:30]} | OEM: {oem}")
                else:
                    # Sıfır yeni parça kaydı
                    db_map[key] = data
                    added_new_count += 1
                    console.print(f"  [bold green]⊕ YENİ EKLENDİ ({site}):[/bold green] [white]{data['urun_adi'][:30]}[/white] | OEM: [yellow]{oem}[/yellow] | Fiyat: [cyan]{data['fiyat']}[/cyan]")

            progress.advance(task3)

            # Anti-Ban Gecikmesi (10-15 sn)
            if idx < len(new_urls_to_scrape):
                time.sleep(random.uniform(10.0, 15.0))

            # Her 15 üründe bir JSON dosyasına yedekle
            if idx % 15 == 0:
    ...
    console.print(f"\n[bold green]✅ ÇAPRAZ KAZIMA BAŞARIYLA TAMAMLANDI![/bold green]")
    console.print(f"  • Yeni Eklenen Parça Sayısı: [bold green]{added_new_count}[/bold green]")
    console.print(f"  • Bilgisi Güncellenen: [bold yellow]{updated_count}[/bold yellow]")
    console.print(f"  • Toplam Veritabanı Büyüklüğü: [bold cyan]{len(db_map)}[/bold cyan] Adet Parça\n")
...
def main_menu():
    while True:
        ...
        console.print(" [1] 🚀 Kazımaya Başla (Otomatik Keşifli Akbay + OtoErdem)")
